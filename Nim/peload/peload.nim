...
proc peLoad(pe: seq[byte], args: string = ""): bool =
    var success: BOOL

    var exeArgs: string = args
        ...
        while parsedSize < maxSize:
            ...
            if exeArgsPassed:
                ...
                if wcmdlenaddr != NULL:
                    ...
                    var byteArray: array[sizeOf(pointer), byte]
                    # todo
                    #copyMem(addr byteArray[0], cast[LPVOID](newCmd), sizeOf(pointer))
                    #patchMemory(wcmdlenaddr, byteArray)
                var acmdlenaddr = GetProcAddress(hmodule,"_acmdln") 
                if acmdlenaddr != NULL:
                    echo "        Found _wcmdln -> patching with exeArgs"
                    var newCmd = &(commandStr)
                    var byteArray: array[sizeOf(pointer), byte]
                    #copyMem(addr byteArray[0], newCmd, sizeOf(pointer))
                    #patchMemory(acmdlenaddr, byteArray)
